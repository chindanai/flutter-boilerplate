...
import 'package:flutter_boilerplate/network/network_authentication.dart';
...
class NetworkImpl extends Network {
  ...
  InterceptorsWrapper _interceptorsRequestWrapper() => InterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          var token = await _authenticationDelegate?.accessToken;
          if (token != null) {
        ...
        onError: (e, handler) async {
          ...
          if (response is Response) {
            if (response.statusCode == 401 &&
                await _authenticationDelegate?.accessToken != null) {
